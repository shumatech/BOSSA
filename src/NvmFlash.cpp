...
NvmFlash::writePage( uint32_t page )
{
  ...
  uint16_t us_status_reg = _samba.readWord(NVM_STATUS_REG) & 0xffff;
  _samba.writeWord(NVM_STATUS_REG, us_status_reg | NVMCTRL_STATUS_MASK);
...
#if 0
  uint32_t start = 0 ;
  for ( uint16_t i=0 ; i < (_size / SAMBA_API_WORD_SIZE); i++ )
  {
